...
def map_school(row: dict[str, Any]) -> School:
    ...
    if any(not value for value in values.values()):
        raise AppError(502, "NEIS_INVALID_RESPONSE", "학교 응답 형식이 올바르지 않습니다.")
...
def parse_servings(value: str | int | float | None) -> int | None:
...
def map_meal(row: dict[str, Any]) -> Meal:
    ...
    try:
        ...
        raise AppError(502, "NEIS_INVALID_RESPONSE", "급식 응답 날짜 형식이 올바르지 않습니다.") from exc
    if not menu_items:
        raise AppError(502, "NEIS_INVALID_RESPONSE", "급식 메뉴 응답 형식이 올바르지 않습니다.")
    return Meal(
        date=meal_date,
