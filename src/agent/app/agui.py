...
from .schemas import AgentError, AnalysisResult, AnalysisState, EvaluationArea
from .workflow import AgentFactory, build_evaluation_workflow, evaluation_prompt
...
class LunchAnalysisAGUIWorkflow(AgentFrameworkWorkflow):
    def __init__(
        ...
        super().__init__(
            ...
            description="Concurrent rubric-based school lunch comparison.",
        )
    ...
    async def run(self, input_data: dict[str, Any]) -> AsyncGenerator[BaseEvent]:
        ...
        try:
            state = AnalysisState.model_validate(input_data.get("state") or {})
        except ValueError:
        ...
        except (LunchDataError, ValueError) as exc:
            async for event in self._user_error_events(state, str(exc)):
                yield event
        except Exception:
            logger.exception(
                "Unhandled lunch analysis failure: thread_id=%s run_id=%s",
                thread_id,
                run_id,
            )
            yield RunErrorEvent(
    ...
    async def _analyze(
        ...
        if len(set(state.selected_school_codes)) != 2:
        ...
        try:
            ...
            raise LunchDataError(
                "화면에 표시된 후보 학교 중 두 곳을 선택해 주세요."
            ) from exc

        yield self._state_event(state, phase="loading_meals", error=None, result=None)
        meal_a, meal_b = await self._data_source.meals_for(
            school_a,
            school_b,
            state.selected_date,
        )
        ...
        workflow = build_evaluation_workflow(
            self._agent_factory,
            {
                area: self._instructions.specialist(area)
                for area in EvaluationArea
            },
            self._instructions.judge(),
            meal_a,
            meal_b,
        )
            ...
            "messages": [
                {
                    ...
                    "content": evaluation_prompt(
                        meal_a,
                        meal_b,
                        _latest_user_message(input_data),
                    ),
                }
            ],
        }
        ...
        async for event in nested.run(nested_input):
            if isinstance(event, (StepStartedEvent, StepFinishedEvent)):
                if (
                    ...
                    and event.step_name == "final-judge"
        ...
        if result is None:
            raise RuntimeError("Evaluation workflow completed without a result.")
        yield self._state_event(state, phase="completed", result=result, error=None)
        async for event in _assistant_message(result.judge.headline):
            yield event
    ...
    async def _user_error_events(
        ...
        error = AgentError(code="DATA_UNAVAILABLE", message=message, retryable=True)
    ...
    @staticmethod
    def _state_event(state: AnalysisState, **updates: Any) -> StateSnapshotEvent:
