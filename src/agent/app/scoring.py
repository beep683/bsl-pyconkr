...
def calculate_school_scores(
    ...
    if set(by_area) != set(AREA_WEIGHTS) or len(by_area) != len(AREA_WEIGHTS):
        raise ValueError("Exactly one evaluation per rubric area is required.")
    ...
    def build(
        ...
        areas = [
            WeightedAreaScore(
                area=area,
                rating=getattr(by_area[area], side).score,
                weight=weight,
                weighted_score=weighted_score(
                    getattr(by_area[area], side).score,
                    weight,
                ),
            )
    ...
    return build(school_a, "school_a"), build(school_b, "school_b")
